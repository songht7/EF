		...
		<view class="uni-navbar__content" :class="{'uni-navbar--fixed': !!fixed,'uni-navbar--shadow':!!border,'uni-navbar--border':!!border}"
		 :style="{'background-color':backgroundColor}">
			...
			<view class="uni-navbar__header" :style="{color:color}">
				...
				<view class="uni-navbar__header-container" :class="{'ctnfixed-end':!!ctnfixed}">
					<view v-if="title.length" class="uni-navbar__header-container-inner">{{title}}</view>
					<!-- 标题插槽 -->
					<slot></slot>
				</view>
				<view class="uni-navbar__header-btns" v-if="rightShow==='true'" @tap="onClickRight">
...
<style>
	...
	@media screen and (min-device-width: 1025px) {
		...
		.uni-navbar__header .uni-navbar__header-btns {
			width: 120px;
			padding: 0 12px;
		}

		.uni-navbar__header .uni-navbar__header-btns:last-child {
			width: 30px;
		}
